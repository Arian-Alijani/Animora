...
| 05 | Clients Module Screens | todo-ready | 0/38 | Step 0 run: `TODO.md` covers `ClientsErrors`, the Owner/Patient/MedicalFileSummary read models, the split read/write seams with their Stage A fakes, seven handlers, the five screens (owner list/form, patient list, patient form, medical-file summary) behind `AddClientsModule()`, the `MutableLWW` sync-class declaration in `Animora.Contracts/Sync`, the module's handler tests and one headless RTL fact per route. Items 2-4 ask the user for the owner/patient field surface, the medical-file header set and the duplicate-mobile rule before the dependent items run (AG-02) |
