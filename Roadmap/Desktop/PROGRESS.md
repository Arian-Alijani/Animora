...
| 05 | Clients Module Screens | in-progress | 6/38 | Items 1-6 done: `ClientsErrors` (`ValidationFailed`, `OwnerNotFound`); items 2-3 answered by the user (owner: address/city/notes/intake date; patient: breed, birth date, weight, sterilization, microchip id + implant date, color, temperament, housing type, diet, barcode, surgical history) and item 4 recorded as a documented default (mobile numbers may be shared across owners); `IOwnerInput`/`OwnerValidator`/`OwnerDto` and `IPatientInput`/`PatientValidator`/`PatientDto` extended accordingly, with a new `PatientValidator.AllowedHousingTypes` registry and the two validator test doubles updated to keep compiling. Items 7-38 (screens, handlers, data seams, `MutableLWW` sync-class declaration, tests) are unchanged/still open |
