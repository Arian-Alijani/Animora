...
| 04 | Identity & Auth Screens | in-progress | 34/41 | Shared Identity validation surfaces (staff, role, credentials) in place, with the Iranian phone formats lifted out of `OwnerValidator` into one internal helper (INV-02); `IdentityErrors` (`ERR-IDENTITY-001..008`), the module's `Mediator`/`Avalonia` package references, and the Stage A read models (`PermissionClaim`/`PermissionCatalog`, `StaffMember`, `Role`, `DeviceRegistration`, `SignedInStaff`) plus the staff, role, device and credential data seams (`IStaffReadStore`/`IStaffWriteStore`, `IRoleReadStore`/`IRoleWriteStore`, `IDeviceReadStore`, `IStaffCredentialReadStore`, keyset paging over `Username`) are in place; the product owner specified the owner-admin username-namespacing rule (SEC-17, `{ownerUsername}-{suffix}`), now recorded in `10-security-and-access-control.md`, `IStaffReadStore.FindOwnerAdminUsernameAsync` and `IdentityErrors.SubordinateUsernamePrefixRequired`, with items 15/22/28/36 amended to enforce and test it once handlers/screens land; all five handlers and all five screens (login, staff list/form, role management, device list) are implemented and wired via `AddIdentityModule()`, now called from the composition root (item 32); sign-in fills `CurrentUserState` through `StaffSignedInNotification`, resolving `CurrentUserState`'s `TODO(P1-04)` (item 33), and the login screen navigates to the shell's landing route on success; `Animora.Desktop.Modules.Identity` is now referenced from both test projects (item 34); the recorded field-format defaults and the six decisions `TODO.md` encodes are open to correction; remaining items are the module's unit/UI tests (35-39) and the two user-run/completion-criteria items (40-41) |
