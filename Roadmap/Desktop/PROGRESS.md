...
| 04 | Identity & Auth Screens | in-progress | 12/41 | Shared Identity validation surfaces (staff, role, credentials) in place, with the Iranian phone formats lifted out of `OwnerValidator` into one internal helper (INV-02); `IdentityErrors` (`ERR-IDENTITY-001..007`), the module's `Mediator`/`Avalonia` package references, and the Stage A read models (`PermissionClaim`/`PermissionCatalog`, `StaffMember`, `Role`, `DeviceRegistration`, `SignedInStaff`) plus the staff and role data seams (`IStaffReadStore`/`IStaffWriteStore`, `IRoleReadStore`/`IRoleWriteStore`, keyset paging over `Username`) are in place; the recorded field-format defaults and the five decisions `TODO.md` encodes (rail-visible Stage A login, module-owned permission-claim catalog, split read/write seams, recorded field-format defaults, sign-in projected onto `ICurrentUserState` through a Mediator notification) are open to correction |
